...
/**
 * The Algorithm 1 class is a implementation of node mapping and link mapping of virtual requests.
 * Algorithm one finds the possible candidate physical nodes containing the requested network function and 
 ...
 * node to the closest candidate node to the <strong>predecessor node</strong>.
		...
		for (int i = 1 ; i < vns.size() - 1; i++){							// no mapping needed for origin and destination
			...
			for(Entry<Integer, PhysicalNode> pn : pns.entrySet()){
				...
				if (currentPN.getNetworkFunction(0) == requestedFunction || currentPN.getNetworkFunction(1) == requestedFunction){
					 ...
					 *              is the closest node to the mapping of the predecessor virtual node. */
